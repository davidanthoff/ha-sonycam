...
if [ -z "${SDK_ZIP}" ]; then
    bashio::log.error "No Sony SDK found. Copy the CrSDK Linux64PC zip to /share/sonycam/ or set the sdk_url option. See the add-on documentation."
    exit 1
fi
bashio::log.info "Using Sony SDK: ${SDK_ZIP}"
...
if [ "${STAMP_NEW}" != "${STAMP_OLD}" ] || [ ! -x "${BUILD_DIR}/sonycam" ]; then
    bashio::log.info "Building sonycam (first run or SDK/source changed) ..."
    rm -rf "${SDK_ROOT}" "${BUILD_DIR}"
    mkdir -p "${SDK_ROOT}"
    unzip -q -o "${SDK_ZIP}" -d "${SDK_ROOT}/zip"
    RCLI_ZIP=$(find "${SDK_ROOT}/zip" -name "RemoteCli.zip" | head -1)
    if [ -z "${RCLI_ZIP}" ]; then
        bashio::log.error "RemoteCli.zip not found inside the SDK zip - is this the Camera Remote SDK Linux 64-bit PC package?"
        exit 1
    fi
